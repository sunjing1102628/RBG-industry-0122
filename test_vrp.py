...
# def judge_instances(model,locs,demands,depot,device):
#     model.to(device)
#     cost=0
#     pi=[]
#     for j in range(len(locs)):
#         batch={}
#         batch['depot']=depot[None,:].to(device)
#         batch['loc']=locs[j][None,:,:].to(device)
#         batch['demand']= demands[j][None,:].to(device)
#     #     print(batch,batch['depot'].shape,batch['loc'].shape, batch['demand'].shape)
#         cost1,l1,pi1=model(batch,return_pi=True,cal_ll=False)
# #         print(cost1)
#         cost=cost+cost1
#         pi.append(pi1)
#     return cost,pi
...
#         U,S,V=torch.pca_lowrank((route_mean-route_mean_mean).transpose(0,1))
#         route_PCA=torch.matmul((route_mean-route_mean_mean).transpose(0,1),V[:,:1])[:,0]+route_mean_mean   # (route_num, )
...
def mean_regions_points(regions1):
    for r in regions1:
        if len(r)!=0:
            mean=np.zeros((len(regions1),r[0].shape[-1]))
            break
...
class SelectModel(nn.Module):
    ...
    def embed_route(self,route):  #route.shape=(route_cnt,3)
        # print(route.device)
        route_emb=self.linear0(route[:,:3])
...
#         print('dist_square.shape=',dist_square.shape)
        ...
        for i in ranges:
            ...
            selected[i]=1
            selected[s[i]]=1
            if not (len(regions2[i])==0 and len(regions2[s[i]])==0): # exclude empty regions
...
def plot_regions0(regions0):
    ...
    plt.savefig('./saved_graph/{}_{}.jpg'.format(args.save_name,graph_count))
    graph_count+=1
    plt.show()
...
def plot_regions1(regions1,divide_num=1,scale=True):
    return
    global graph_count, args
    color_base=np.array([0,0,0])
...
#         print('sorted_route_PCA=',sorted_route_PCA)
        ...
        for idx in sorted_route_PCA[ii:]:
            sub2.append(r1[idx])
        regions2.append(sub1)
        regions2.append(sub2)
...
def intersect_num(region1,region2):
    if len(region1)==0 or len(region2)==0:
        return 0
    region1=torch.cat(region1,0)[:,3].cpu().long()
    region2=torch.cat(region2,0)[:,3].cpu().long()
    max1=max(torch.max(region1),torch.max(region2))

    visited=torch.zeros(max1+1)
#     print(max1,torch.max(region1),torch.max(region2),visited.shape[0])
    visited[region1]=1
    cnt=0
    for i in region2:
        if visited[i]==1:
...
# ccc=0
def divide_and_update_region(regions1,merged_regions2,should_update,return_reallocation=False):
#     global ccc
    regions2=[]
    sum_reallocation_num=0
    sum_nodes=0
#     plot_regions1(regions1, scale=False)
    for cnt1,r1 in enumerate(regions1):  #[(route_len,3)]*route_num
        if should_update[cnt1]:
#             plot_region(regions1[cnt1])
#             print('len=',len(regions1[cnt1]))
#             print('r1=',r1)
            route_mean=torch.stack([torch.mean(route,0)[:2] for route in r1],0) #(route_num,2)
    #         print('route_mean=',route_mean.shape)
            route_cnt=[route.shape[0] for route in r1]   #(route_num,)
    ...
    #         print('sorted_route_PCA=',sorted_route_PCA)
            ...
            for i,idx in enumerate(sorted_route_PCA):
                ...
                sub1.append(r1[idx])
                ii+=1
#             print(sorted_route_PCA,i)
            for idx in sorted_route_PCA[ii:]:
                sub2.append(r1[idx])
            # sum_nodes+=count_num_region(regions1[cnt1])
#             ccc=ccc+1
#             print(ccc)
            # assert count_num_region(sub1)+count_num_region(sub2)==count_num_region(regions1[cnt1]), 'divide problem!,{},{},{}'.format(count_num_region(sub1),count_num_region(sub2),count_num_region(regions1[cnt1]))
            # assert count_num_region(regions1[cnt1])==count_num_region(merged_regions2[cnt1*2])+count_num_region(merged_regions2[cnt1*2+1]), 'count num not the same,{},{},{}'.format(count_num_region(regions1[cnt1]),count_num_region(merged_regions2[cnt1*2]),count_num_region(merged_regions2[cnt1*2]))
            # reallocation_num=min(intersect_num(sub1,merged_regions2[cnt1*2])+intersect_num(sub2,merged_regions2[cnt1*2+1]),intersect_num(sub2,merged_regions2[cnt1*2])+intersect_num(sub1,merged_regions2[cnt1*2+1]))
            # sum_reallocation_num+=reallocation_num
        if not should_update[cnt1]: #worse than previous solution, therefore not updating
        ...
        regions2.append(sub2)
#     print('len=',len(regions2))
    if return_reallocation:
        return regions2, sum_reallocation_num, sum_nodes
    else:
        return regions2
...
def eval(data1,model,selectmodel,args,always_update=False,
    ...
    record_regionss2=[]
    record_costss=[]
    iter_sum_reallocation_num=np.zeros(iter_step)
    iter_sum_points=np.zeros(iter_step)
    iter_region_len_distribution=[[] for i in range(iter_step)]
    iter_updated_region_num=np.zeros(iter_step)
    iter_propose_region_num=np.zeros(iter_step)
    for n in (range(num_samples//multiple+1)):
        ...
        nodes=[torch.cat((loc[i],demand[i][:,None],torch.arange(1,loc[i].shape[0]+1,dtype=torch.float)[:,None]),-1) for i in range(len(demand))]     #[(N,3)]*multiple
        print('shape=',nodes[0].shape)
...
#                 print('costs1.shape=',costs1.shape,merged_costs1.shape,len(regions1))
                ...
                regions2, sum_reallocation_num,sum_points=divide_and_update_region(regions1,merged_regions2,should_update,return_reallocation=True)
                # iter_sum_reallocation_num[step]+=sum_reallocation_num
                # iter_sum_points[step]+=sum_points
                # iter_updated_region_num[step]+=torch.sum(should_update.int()).item()
                # iter_propose_region_num[step]+=len(regions1)
                # print('sum_region_num=',torch.sum(should_update.int()).item(),len(regions1),iter_updated_region_num[step]/iter_propose_region_num[step])
                # print('sum_points=',sum_reallocation_num,sum_points,iter_sum_reallocation_num[step]/iter_sum_points[step])
            ...
            # for r1 in regionss2:
            #     for r2 in r1:
            #         region_len.append(count_num_region(r2))
            # iter_region_len_distribution[step].append(region_len)
...
#             t5=time.time()
            ...
            print('data{},step{}:{},{}'.format(n,step,costs,costs.mean()))#  sample cost,shape=(multiple,)
    ...
    if record_iter_steps is not None:
        ...
        return torch.cat(final_cost), record_regionss2, record_costss,iter_sum_reallocation_num,\
                iter_sum_points,iter_updated_region_num,iter_propose_region_num,iter_region_len_distribution
    #     print(regions1)
    else:
    ...
    #     record_regionss2=[]
        ...
        if not args.train_selection_only:
            ...
            if cost2<cost1:
                ...
                best_val_cost=cost1
                best_id=epoch
        print('epoch={}, val_cost={}, val_cost2={}, best_epoch={}'.format(epoch,cost1.item(),cost2.item(),best_id))
        with open('log_{}.txt'.format(args.save_model),'a') as f:
            f.write('epoch={}, val_cost={}, val_cost2={}, best_epoch={}\n'.format(epoch,cost1.item(),cost2.item(),best_id))
        # fixed_model.load_state_dict(model.state_dict())
        # fixed_model.eval()
        # set_decode_type(fixed_model, "greedy")
...
def parse():
    ...
    parser.add_argument('--epoch_size',default=100, type=int, help='random seed.')
    parser.add_argument('--num_epochs',default=100, type=int, help='random seed.')
    parser.add_argument('--batch_size',default=10, type=int, help='random seed.')
    parser.add_argument('--eval_batch_size',default=20, type=int, help='random seed.')
    parser.add_argument('--val_size',default=100, type=int, help='random seed.')
    parser.add_argument('--near_K',default=5, type=int, help='random seed.')
    parser.add_argument('--save_epoch',default=10, type=int, help='random seed.')
    parser.add_argument('--K_means_step',default=10, type=int, help='random seed.')
    parser.add_argument('--iter_step_train',default=10, type=int, help='random seed.')
    parser.add_argument('--iter_step_eval',default=10, type=int, help='random seed.')
    parser.add_argument('--size',default=500, type=int, help='random seed.')
    parser.add_argument('--lr1',default=1e-4, type=float, help='random seed.')
    parser.add_argument('--lr2',default=1e-4, type=float, help='random seed.')
    parser.add_argument('--beta',default=0.1, type=float, help='random seed.')
    parser.add_argument('--always_update',default=False,action='store_true')
    parser.add_argument('--proctitle',default='test3@whs',type=str)
    ...
    parser.add_argument('--update_model_step',default=2,type=int)
    parser.add_argument('--model_type',default=2,type=int)
...
if __name__ == '__main__':
    args=parse()
    print(args.enable_running_cost)
    args.random_selection=False

    print(args.always_update)
    if args.always_update==False:
        print('daffafn')
...
#     with open('log_{}.txt'.format(args.save_model),'w') as f:
#         f.write('0\n')
    ...
    path_100=[
        'pretrained/cvrp_100/epoch-99.pt',               #
        'outputs/cvrp_100/route5_20200503T170932/epoch-32.pt',  #103.08
        'outputs/cvrp_100/route4_20200501T042118/epoch-48.pt',  #99.05,3s
        'outputs/cvrp_100/route3_20200501T042021/epoch-32.pt',  #100.64
        'outputs/cvrp_100/route2_20200501T041458/epoch-40.pt'   #99.54
             ]


    model_100, _ = load_model(path_100[2])
    model_100.to(device)
    ...
    # betas=[0,0.1,0.2,0.5,1,10]
    betas=[0.1]
    # betas=[0.5,1,2,5,10,10000000]
    iter_steps=list(range(1,100))
    # iter_steps=[1,2,3]
    batch_sizes={500:100,1000:100,2000:50}
    # batch_sizes={500:15,1000:6,2000:2}
    # record_max_epoch1=torch.zeros(len(ids),len(sizes))
    record_costs1=torch.zeros(len(sizes),len(sizes),len(iter_steps),num_samples)
    record_mean_cost1=torch.zeros(len(sizes),len(sizes),len(iter_steps))
    record_iter_reallocate=torch.zeros(len(sizes),len(sizes),len(iter_steps))
    record_mean_time1=torch.zeros(len(sizes),len(sizes))
    record_iter_reallocate=torch.zeros(len(sizes),len(sizes),len(iter_steps))
    record_iter_update_rate=torch.zeros(len(sizes),len(sizes),len(iter_steps))
    len_dis=[]



    # filename='test_{}_{}_{}_{}.pkl'.format(size,num_samples,CAPACITIES,data_type)
    # print(filename)
    # with open(filename,'rb') as f:
    #     data1=pickle.load(f)
    ...
    model_id=41
    # rescale_size=np.zeros((3, num_samples))
    for model_size_id,model_size in enumerate(sizes):
        for size_id,size in enumerate(sizes):
            if size==model_size:
                continue
            path='saved/model{}_{}_100.pkl'.format(model_size,41)

            filename='data/data_{}_{}_{}.pkl'.format(size,num_samples,CAPACITIES)
            print(filename)
            with open(filename,'rb') as f:
                data=pickle.load(f)
            ...
            args.beta=0.1
            if model_id==41:
                args.near_K=near_Ks[size_id]
            else:
                args.near_K=100
            args.iter_step_eval=iter_steps[-1]
            args.eval_batch_size=batch_sizes[size]
            print('size=',size,'model_id=',model_id,'model_size=',model_size)
    #         if size_id<2:
    #             continue
            torch.manual_seed(seed)
            random.seed(seed)
            ...
            selectmodel.load_state_dict(a2)
            selectmodel.to(device)
            selectmodel.near_K=near_Ks[size_id]
    ...
    #             selectmodel.load_state_dict(b)
            t10=time.time()
            print(args.up_rate_eval)
            args.up_rate_eval=0
            args.up_rate_train=0
            costs,regionss2,iter_costs,num1,num2,num3,num4,len_distribution=eval(data,model,selectmodel,args=args,
                                                      always_update=False,record_iter_steps=iter_steps )

            # for regions in regionss2:
            #     re=[]
            #     for region in regions:
            #         if len(region)!=0:
            #             a=torch.cat(region,0)
            #             re.append(a)
            #     assert ((torch.cat(re,0)[:,3].sort()[0]==torch.arange(1,size+1)).all()),''

            print(iter_costs.shape,iter_costs,costs)
            # print('num1=',num1,num2,num1/num2,num3,num4,num3/num4)
            # record_iter_reallocate[model_size_id,size_id,:]=torch.tensor(num1/num2)
            # record_iter_update_rate[model_size_id,size_id,:]=torch.tensor(num3/num4)
            record_mean_time1[model_size_id,size_id]=(time.time()-t10)/num_samples
            record_costs1[model_size_id,size_id,:,:]=iter_costs
            record_mean_cost1[model_size_id,size_id,:]=torch.mean(iter_costs,-1)
            # len_dis.append(len_distribution)

            print('mean_cost=',torch.mean(iter_costs,-1))


    # [record_iter_reallocate,record_iter_update_rate,record_mean_time1,record_costs1,record_mean_cost1,len_dis]


    save_data('{}.pkl'.format(args.save_name),[record_iter_reallocate,record_iter_update_rate,record_mean_time1,record_costs1,\
                         record_mean_cost1,len_dis ])
    # [record_iter_reallocate2,record_iter_update_rate2,record_mean_time2,record_costs2,\
    #                      record_mean_cost2,len_dis2 ]=load_data('trained_model.pkl')
    print(record_iter_reallocate)
