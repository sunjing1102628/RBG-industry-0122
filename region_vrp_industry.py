...
def K_means(K,node,beta=0,depot=torch.tensor([0.5,0.5]),step=10,plot=False,device=torch.device('cpu')):
    ...
    loc=node[:,:2]
    assert loc.nelement() > 0, "The loc tensor is empty!"
    assert loc_center.nelement() > 0, "The loc_center tensor is empty!"
...
def generate_industry_data(file_directory, device=torch.device('cpu')):
    with open(file_directory, 'r') as f:
        lines = f.readlines()
    lines = lines[1:1001]   
    CAPACITIES = 150  # adjust as per your needs
    ...
    min_lat, max_lat, min_lon, max_lon = find_min_max_lat_lon(file_directory)
    ...
    for line in lines:
        ...
        try:
            ...
            corrected_data = fields[5].replace('""', '"')
            ...
            print("Error with data:", fields[5])
            raise e
        depot_lat, depot_lon = normalize_lat_lon(1.332486, 103.707966, min_lat, max_lat, min_lon, max_lon)
        # node_demand = torch.tensor([demand_value]) / CAPACITIES
        print(norm_lat, norm_lon)
        locationObject= [norm_lat, norm_lon]
        locationArray.append(locationObject)
        demandArray.append(demandObject / CAPACITIES)
    sample = [{
            'demand': torch.tensor(demandArray),
            'depot': torch.tensor([depot_lat,depot_lon]),
            'loc': torch.tensor(locationArray)
        }]
    

    return sample
def generate_industry_services_data(file_directory, device=torch.device('cpu')):
    with open(file_directory, 'r') as f:
        lines = f.readlines()
    lines = lines[1:3301]   
    ...
    return sample
def find_min_max_lat_lon(file_directory):
    with open(file_directory, 'r') as f:
        lines = f.readlines()
...
def eval(data1,model,selectmodel,args,always_update=False,
    ...
    print('number sample', num_samples)
    ...
    for n in (range(num_samples//multiple+1)):
        ...
        for i in range(len(demand)):
            print(demand[i].shape, loc[i].shape)

        nodes=[torch.cat((loc[i],demand[i][:,None]),-1) for i in range(len(demand))]     #[(N,3)]*multiple


        regionss0=[K_means(K,node,depot=depot,beta=beta,step=K_means_step,plot=True) for node in nodes] #[[(num_in_cluster(diff), 3)]*K]*multiple
...
def write_routes_to_excel(record_regionss2, filename):
    # Create a new Excel writer objects
    writer = pd.ExcelWriter(filename, engine='openpyxl')

    for sample_idx, routes in enumerate(record_regionss2):
        # Create a DataFrame for each sample's routes
        all_routes = []
        for route_idx, route in enumerate(routes):

            locations = route.numpy()[:, :2].tolist()  # Assuming the first 2 columns are the x,y coordinates
            all_routes.append(['Route ' + str(route_idx)] + [str(loc) for loc in locations])
        
        # Find the longest route to determine the number of columns
        max_len = max(len(r) for r in all_routes)

        # Pad shorter routes with NaN so that the dataframe is rectangular
        for r in all_routes:
            while len(r) < max_len:
                r.append(float('nan'))

        # Convert the routes to a DataFrame
        df = pd.DataFrame(all_routes).transpose()

        # Write the dataframe to the Excel writer
        df.to_excel(writer, sheet_name='Sample_' + str(sample_idx), index=False, header=False)
...
if __name__ == '__main__':
    ...
    # Example usage
    file_directory = "test_instances/Industry_data_test/parcel.txt"
    #transformed_data = generate_industry_data(file_directory)
    transformed_data = generate_industry_services_data(file_directory)
    print("dones")
    ...
    costs,record_regionss2=eval(transformed_data,model,selectmodel,args,always_update=False)
    print('epoch=0, val_cost=',costs.mean())
    # Use the function
    write_routes_to_excel(record_regionss2, 'routes.xlsx')
