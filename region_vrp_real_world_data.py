...
'''def stacks(li,device=None,dim=0,stack_dim=0,min_length=None,constant=0):
    ...
    if min_length is not None:
        m=max(m,min_length)
    li1 = []
    pad_dim=[0]*len(li0[0].shape)*2
    size_n=torch.tensor([s.shape[dim] for s in li0])
    print('size_n111111111',size_n)
...
def merge_and_judge_instances(model, region0, depot, device, cal_ll=False, args=None):
    # print('regions0&&&&&&&&&',region0) #8
    regions0 = []
    for i in range(len(region0)):
        if region0[i] != None:
            regions0.append(region0[i])

    # print('region0^^^^', len(regions0)) #torch.Size([171, 3])

    nodes, size_n = stacks(regions0, device, constant=0)  # (K,num_in_cluster,3)
    # print('size_n2222222', size_n.shape)
    ...
    with torch.no_grad():
        ...
        # print('size$$$$$$$$=',size_n.shape,batch['loc'].shape) #torch.Size([4]) torch.Size([4, 267, 2])
...
def generate_data(num_samples=10000, size=500, device=torch.device('cpu')):
    CAPACITIES = 50
    print('demand_train', ((torch.FloatTensor(size).uniform_(0, 9).int() + 1).float() / CAPACITIES).size())
    print('loc', )
...
def generate_test_data(path="./", device=torch.device('cpu')):
    path = 'VRP_Instances_Soloman/C1_7_1.120.120.txt'
    file = open(path, "r")
    ...
    print('original_locations', torch.tensor(original_locations).size())

    node_demand = torch.Tensor(demand[:, 1:].reshape((1, -1))) / capacity  # [1, n]

    node_demand0 = node_demand.squeeze(0).t()
    print('node_demnad0', node_demand0.size())
    ...
    print('loc***********', torch.tensor(original_locations).dtype)
    print('depot$$$$', torch.tensor([40.0, 40.0]).dtype)

    return [{'depot': torch.tensor([40.0, 40.0]), 'loc': torch.tensor(original_locations),
             'demand': node_demand0}]
...
def eval(data1, model, selectmodel, args, always_update=False,
    ...
    for n in (range(num_samples // multiple + 1)):
        ...
        depot = [data1[i]['depot'] for i in
                 range(n * multiple, min((n + 1) * multiple, num_samples))]  # [(N,2)]*multiple
        demand = [data1[i]['demand'] for i in
                  range(n * multiple, min((n + 1) * multiple, num_samples))]  # [(N,)]*multiple

        # print('depot_test222',depot[0].size()) #torch.Size([2])

        nodes = [torch.cat((loc[i], demand[i][:, None]), -1) for i in range(len(demand))]  # [(N,3)]*multiple
        # print('nodes!!!',len(nodes))
        # print('node22', nodes[0].size()) #torch.Size([801, 3])
        regionss0 = [K_means(K, node, beta=beta, step=K_means_step, plot=False) for node in
                     nodes]  # [[(num_in_cluster(diff), 3)]*K]*multiple
        ...
        for step in range(iter_step):
            ...
            #             t1=time.time()
            # print('depot[0]',depot[0])
            # print('regions033333333',len(regions0))
            regions1a = []
            for i in range(len(regions0)):
                if regions0[i] != None:
                    regions1a.append(regions0[i])
            costs1, loss, pi = merge_and_judge_instances(model, regions1a, depot[0], device=torch.device('cuda'),
                                                         cal_ll=False, args=args)
            # costs.shape=(N,); loss.shape=(N,); pi.shape=(N,max_len)
            #             print('time3=',time.time()-t1)
            #             t1=time.time()
            # print('test$$$$$',pi.shape[0])

            # print('pi.shape=',pi.shape,pi.device,len(regions1a))

            regions1 = pi2regions1(regions1a, pi)  # [[(_route_len,3)]*route_num_in_a_region]*num_region, 100 points
            ...
            if step == 0 or always_update:
                ...
                costss1 = parse_idx(torch.where(should_update, costs1, merged_costs1), idx_regions1,
                                    dim=0)  # [(num_regions1)]*multiple

            costs = torch.tensor([c.sum().item() for c in costss1]) + remain_costs  # (multiple,)

            for i in range(len(nodes)):
                regionss2[i] = regions2[idx_regions1[i] * 2:idx_regions1[i + 1] * 2] + remain_regionss2[
                    i]  # [[(_route_len,3)]*route_num_in_a_region]*num_region, 50 points
            ...
            for i in range(len(nodes)):
                ...
                #                 t1=time.time()
                regions1, regions2, selected, logits2_i, merged_costs1_i, merged_regions2_i = selectmodel.merge_regions(
                    regionss2[i], cov, costs2)  # selected.shape=(2K,)
...
def train_batch(nodes, optimizer, model, fixed_model, selectmodel, fixed_selectmodel, always_update=False, args=None,
                depot=None, batch_id=None, running_cost=0):
    ...
    for step in range(iter_step):
        ...
        if args.train_selection_only:
            ...
            costs1_sample, ll, pi, costs1 = merge_and_train_instances(model, fixed_model, regions0, depot[0], device,
        ...
        for i in range(len(nodes)):
            regionss2[i] = remain_regionss2[i] + regions2[idx_regions1[i] * 2:idx_regions1[
                                                                                  i + 1] * 2]  # [[(_route_len,3)]*route_num_in_a_region]*num_region, 50 points
        #             plot_regions1(regions2,divide_num=2)
    ...
    for epoch in range(num_epochs):
        ...
        for n in range(epoch_size // batch_size + 1):
            if n * batch_size == epoch_size:
                continue
            loc = [data[i]['loc'] for i in
                   range(n * batch_size, min((n + 1) * batch_size, epoch_size))]  # [(N,2)]*multiple
            depot = [data[i]['depot'] for i in
                     range(n * batch_size, min((n + 1) * batch_size, epoch_size))]  # [(N,2)]*multiple
            demand = [data[i]['demand'] for i in
                      range(n * batch_size, min((n + 1) * batch_size, epoch_size))]  # [(N,)]*multiple
            nodes = [torch.cat((loc[i], demand[i][:, None]), -1) for i in range(len(demand))]  # [(N,3)]*multiple
            # print('test%%%%%%',depot[0])

            running_cost = train_batch(nodes, optimizer, model, fixed_model, selectmodel, fixed_selectmodel, args=args,
                                       always_update=args.always_update, depot=depot, batch_id=n,
...
def parse():
    ...
    parser.add_argument('--num_epochs', default=50, type=int, help='random seed.')
    ...
    parser.add_argument('--lr1', default=2.5e-5, type=float, help='random seed.')
    parser.add_argument('--lr2', default=5e-5, type=float, help='random seed.')
    ...
    parser.add_argument('--proctitle', default='test2@whs', type=str)
    ...
    parser.add_argument('--enable_random_rotate_train', action='store_true', default=False)
    parser.add_argument('--enable_random_rotate_eval', action='store_true', default=False)
    parser.add_argument('--up_rate_train', default=0.01, type=float)
    parser.add_argument('--up_rate_eval', default=0.005, type=float)
    parser.add_argument('--rl_steps', default=3, type=int)
    parser.add_argument('--enable_gradient_clipping', action='store_true', default=False)
    ...
    parser.add_argument('--fix_select_model', default=False, action='store_true')
    parser.add_argument('--enable_running_cost', default=False, action='store_true')
...
if __name__ == '__main__':
    ...
    test_dataset = generate_test_data(path="./", device=device)
    ...
    if args.train_from_scratch:
        ...
        path_100 = ['pretrained/cvrp_100/epoch-99.pt']
    ...
    class SelectModel(nn.Module):
        def __init__(self, embed_dim0=100, lstm_dim=100, embed_dims1=[1000, 100], args=None,
    ...
    def cost_of_route(route, depot):
        ...
        #     print(torch.sum(torch.sqrt(torch.sum((route[1:,:2]-route[:-1,:2])**2,1)),0),'\n')
        c = torch.sum(torch.sqrt(torch.sum((route[1:, :2] - route[:-1, :2]) ** 2, 1)), 0)
        return torch.sqrt(torch.sum((route[0, :2] - depot) ** 2)) + c + torch.sqrt(
            torch.sum((route[-1, :2] - depot) ** 2))
    ...
    def train_batch(nodes, optimizer, model, fixed_model, selectmodel, fixed_selectmodel, always_update=False,
                    args=None, depot=None, batch_id=None, running_cost=0):
        batch = args.batch_size
        iter_step = args.iter_step_train
        beta = args.beta
        K_means_step = args.K_means_step
        ...
        for step in range(iter_step):
            ...
            if args.train_selection_only:
                costs1, ll, pi = merge_and_judge_instances(fixed_model, regions0, depot[0], device, cal_ll=True,
                                                           args=args)

            else:
                costs1_sample, ll, pi, costs1 = merge_and_train_instances(model, fixed_model, regions0, depot[0],
                                                                          device, cal_ll=True, args=args)
                # costs.shape=(N,); loss.shape=(N,); pi.shape=(N,max_len)
                loss1 = ((costs1_sample - costs1) * ll).mean()
                # print('cost_sample=',costs1_sample.mean().item(),'  cost1=',costs1.mean().item())
            # print('pi.shape=', pi.shape, pi.device, len(regions0))
    ...
    def train(model, fixed_model, selectmodel, fixed_selectmodel, args, val_dataset):
        ...
        for epoch in range(num_epochs):
            ...
            for n in range(epoch_size // batch_size + 1):
                ...
                running_cost = train_batch(nodes, optimizer, model, fixed_model, selectmodel, fixed_selectmodel,
                                           args=args, always_update=args.always_update, depot=depot, batch_id=n,
                                           running_cost=running_cost)
                # if (n+1)%args.update_model_step==0:
                #     fixed_model.load_state_dict(model.state_dict())
                #     fixed_model.eval()
            ...
            if not args.train_selection_only:
                ...
                if cost2 < cost1:
                    ...
                    best_id = epoch
            print(
                'epoch={}, val_cost={}, val_cost2={}, best_epoch={}'.format(epoch, cost1.item(), cost2.item(), best_id))
            with open('log_{}.txt'.format(args.save_model), 'a') as f:
                f.write('epoch={}, val_cost={}, val_cost2={}, best_epoch={}\n'.format(epoch, cost1.item(), cost2.item(),
                                                                                      best_id))
            # fixed_model.load_state_dict(model.state_dict())
    ...
    if __name__ == '__main__':
        ...
        model.eval()
        selectmodel.eval()
        costs, _ = eval(test_dataset, model, selectmodel, args, always_update=False)
        print('test_cost=', costs)
